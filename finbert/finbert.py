...
class FinBert(object):
    ...
    def train(self, train_examples, model):
        ...
        for _ in trange(int(self.config.num_train_epochs), desc="Epoch"):
            ...
            logger.info(f"Validation losses: {self.validation_losses}")
...
# RPC added the base_model parameter so we can download bert-base-uncased
def predict(text, model, write_to_csv=False, path=None, base_model="bert-base-uncased"):
    """
